...
static unsigned int ceiling_slack_ms = MSEC_PER_SEC;/*1s*/
...
static int set_clus_affinity_uidlist(const char *buf, const struct kernel_param *kp)
{
	int i, len, ntokens = 0;
	unsigned int val;
	const char *cp = buf;
...
module_param_cb(add_lclus_affinity_uidlist, &param_ops_clus_affinity_uidlist, NULL, 0644);
module_param_cb(add_mclus_affinity_uidlist, &param_ops_clus_affinity_uidlist, NULL, 0644);
module_param_cb(add_bclus_affinity_uidlist, &param_ops_clus_affinity_uidlist, NULL, 0644);
module_param_cb(reset_clus_affinity_uidlist, &param_ops_clus_affinity_uidlist, NULL, 0644);
module_param_cb(del_lclus_affinity_uidlist, &param_ops_clus_affinity_uidlist, NULL, 0644);
module_param_cb(del_mclus_affinity_uidlist, &param_ops_clus_affinity_uidlist, NULL, 0644);
module_param_cb(del_bclus_affinity_uidlist, &param_ops_clus_affinity_uidlist, NULL, 0644);
module_param_cb(show_lclus_affinity_uidlist, &param_ops_clus_affinity_uidlist, NULL, 0644);
module_param_cb(show_mclus_affinity_uidlist, &param_ops_clus_affinity_uidlist, NULL, 0644);
module_param_cb(show_bclus_affinity_uidlist, &param_ops_clus_affinity_uidlist, NULL, 0644);
...
static int freq_update_should_skip(enum cluster_type cluster)
{
	...
	case NO_MIGT:
		break;
	case MIGT_1_0:/*Boost mid cluster in migt 1.0*/
		if (cluster == MID_CLUSTER)
			return 0;
		break;
	case MIGT_2_0:/*Boost mid & big cluster in migt 2.0 && 3.0*/
	case MIGT_3_0:
		if (cluster >= MID_CLUSTER)
			return 0;
		break;
...
static void update_policy_online(void)
{
	...
	for_each_online_cpu(i) {
		...
		cpumask_and(active_cluster_cpus,
				cluster_cpus, cpu_online_mask);
...
static void do_ceiling_rem(struct work_struct *work)
{
	...
	if (!ceiling_freq_limit_work)
        ...
        if (migt_debug == 1)
                pr_err("migt do ceiling_rem....%d %llu",
                                __LINE__, ktime_to_us(ktime_get()));
...
static int check_uid_valid(int uid)
{
	int i, users = (ceiling_users.count > MAX_CEILING_USERS) ?
		MAX_CEILING_USERS : ceiling_users.count;
...
static void add_uid_to_ceiling_user(int uid)
{
	...
	if (migt_debug == 2) {
		printk("--------ceiling_user list--------\n");
		for (i = 0; i < users; i++)
			printk ("%d ", ceiling_users.uid[i]);
	}
...
static void do_ceiling_limit(struct work_struct *work)
{
	...
	if (!ceiling_freq_limit_work)
        ...
        if (migt_debug == 2)
                pr_err(
		    "migt ceiling_limit ...%d %llu, ceiling_freq: %d",
		    __LINE__, ktime_to_us(ktime_get()),
		    i_migt_info->ceiling_freq);
...
static void vtask_boost_in_hrtimer(int timeout_ms)
{
	...
	hrtimer_start(&vtask_timer, ms_to_ktime(timeout_ms),
		HRTIMER_MODE_REL);
}
...
static int get_bucket_index (u64 fps_exec)
{
	...
	if (!migt_fast_div)
		migt_fast_div =
			NUM_MIGT_BUCKETS * MSEC_PER_SEC / migt_thresh;
...
void migt_hook(struct task_struct *tsk, u64 delta, int cpu)
{
	...
	if (((tsk->pkg.migt.bucket[index] * fps_variance_ratio
	    < tsk->pkg.migt.bucket[0])) && !migt_boost_flag) {
		if (game_ip_task(tsk)
